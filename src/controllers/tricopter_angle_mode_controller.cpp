...
void TricopterAngleModeController::Update(const ControllerInput &input, MotorCommands &motor_commands) {
  ...
  if (input.timestep_seconds > 0.0f) {
    setpoint_.yaw_rad = std::remainder( 
        setpoint_.yaw_rad + YawRateRadiansPerSecond *
        KeyAxis(keys.q, keys.e) * 
        input.timestep_seconds,
        2.0f * Pi);

    setpoint_.throttle = std::clamp(
        setpoint_.throttle + ThrottleRatePerSecond * 
...
float TricopterAngleModeController::GetThrottle() const { return setpoint_.throttle; }
